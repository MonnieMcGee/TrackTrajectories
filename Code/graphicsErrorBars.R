...
# plot best time vs. grade for girls' 200m times to get a sense of the variability among paths
set.seed(1234)
pr_lm = lm(BestTime ~ Grade, data=fourYears200longG)
ng200 <- dim(fourYears200longG)[1]/4
fourYears200longG$Athlete <- rep(1:ng200, each=4) 
# sample_students = fourYears200longG %>% filter(Name %in% sample(1:ng200, 10))
# grade_sample = fourYears200longG$Grade[fourYears200longG$Name %in% sample_students$Name]
# pr_sample = fourYears200longG$BestTime[fourYears200longG$Name %in% sample_students$Name]
samp200g <- sample(1:ng200, 10)
init = fourYears200longG %>%
  modelr::add_predictions(pr_lm, var = 'all') %>%
  mutate(select = factor(Athlete %in% samp200g),
         sz = c(.5, 1)[select]) %>%
...
init %>%
  ...
  layout(title = 'Best Times for each Girl by Grade (200m)', 
         xaxis = list(title="",
           ticktext=list("Ninth", "Tenth", "Eleventh", "Twelfth"),
               tickvals=list(9, 10, 11, 12)),
         yaxis= list(title = "Fastest Time per Grade (seconds)",
                     autorange="reversed")) %>%
  theme_plotly()
#  in bold. The overall trend, as estimated by a simple linear regression of 
#  best time on grade, is shown in red. 

# Same thing for boys
set.seed(1234)
pr_lm = lm(BestTime ~ Grade, data=fourYears200longB)
nb200 <- dim(fourYears200longB)[1]/4
fourYears200longB$Athlete <- rep(1:nb200, each=4) 
samp200b <- sample(1:nb200, 10)
init = fourYears200longB %>%
  modelr::add_predictions(pr_lm, var = 'all') %>%
  mutate(select = factor(Athlete %in% samp200b),
         sz = c(.5, 1)[select]) %>%
  group_by(Athlete, select) 
init %>%
  plot_ly %>%
  ...
  layout(title = 'Best Times for each Boy by Grade (200m)', 
         xaxis = list(title="",
                      ticktext=list("Ninth", "Tenth", "Eleventh", "Twelfth"),
                      tickvals=list(9, 10, 11, 12)),
         yaxis= list(title = "Fastest Time per Grade (seconds)",
...
## And 1600m 
set.seed(1234)
pr_lm = lm(BestTime ~ Grade, data=fourYears1600longG)
ng1600 <- dim(fourYears1600longG)[1]/4
samp1600g <- sample(1:ng1600, 10)
fourYears1600longG$Athlete <- rep(1:ng1600, each=4) 
init = fourYears1600longG %>%
  modelr::add_predictions(pr_lm, var = 'all') %>%
  mutate(select = factor(Athlete %in% samp1600g),
         sz = c(.5, 1)[select]) %>%
  group_by(Athlete, select) 
init %>%
  plot_ly %>%
  ...
  layout(title = 'Best Times for each Girl by Grade (1600m)', 
         xaxis = list(title="",
                      ticktext=list("Ninth", "Tenth", "Eleventh", "Twelfth"),
                      tickvals=list(9, 10, 11, 12)),
         yaxis= list(title = "Fastest Time per Grade (seconds)",
...
set.seed(1234)
pr_lm = lm(BestTime ~ Grade, data=fourYears1600longB)
nb1600 <- dim(fourYears1600longB)[1]/4
fourYears1600longB$Athlete <- rep(1:nb1600, each=4) 
samp1600b <- sample(1:nb1600, 10)
init = fourYears1600longB %>%
  modelr::add_predictions(pr_lm, var = 'all') %>%
  mutate(select = factor(Athlete %in% samp1600b),
         sz = c(.5, 1)[select]) %>%
  group_by(Athlete, select) 
init %>%
  plot_ly %>%
  ...
  layout(title = 'Best Times for each Boy by Grade (1600m)', 
         xaxis = list(title="",
                      ticktext=list("Ninth", "Tenth", "Eleventh", "Twelfth"),
                      tickvals=list(9, 10, 11, 12)),
         yaxis= list(title = "Fastest Time per Grade (seconds)",
...
gmeans <- slopes200a %>% group_by(Gender, Grade) %>% summarise(mean = median(Slope,na.rm=T)*100)
gmeans$mean <- sprintf("Median Percent Change = %2.1f", gmeans$mean)
ggplot(data=slopes200a,aes(x=Slope)) + 
  geom_histogram(bins=50, fill="azure3")  + theme_bw() + geom_vline(xintercept = 0, linetype="dashed") +
  labs(title="Difference in Times for 200m by Gender and Grade", x="Difference in Times between Consecutive Grades on Log Scale", y=NULL) + 
...
gmeans <- slopes400a %>% group_by(Gender, Grade) %>% summarise(mean = median(Slope,na.rm=T)*100)
gmeans$mean <- sprintf("Median Percent Change = %2.1f", gmeans$mean)
ggplot(data=slopes400a,aes(x=Slope, fill=Gender)) + 
  geom_histogram(bins=50, fill="azure3")  + theme_bw() + geom_vline(xintercept = 0, linetype="dashed") +
  labs(title="Difference in Times for 400m by Gender and Grade", x="Difference in Times between Consecutive Grades on Log Scale", y=NULL) + 
  facet_grid(Grade~Gender, labeller=labeller(Grade = labels)) + theme(legend.position = 'none') +
...
gmeans <- slopes800a %>% group_by(Gender, Grade) %>% summarise(mean = median(Slope,na.rm=T)*100)
gmeans$mean <- sprintf("Median Percent Change = %2.1f", gmeans$mean)
ggplot(data=slopes800a,aes(x=Slope, fill=Gender)) + 
  geom_histogram(bins=30, fill="azure3")  + theme_bw() + geom_vline(xintercept = 0, linetype="dashed") +
  labs(title="Difference in Times for 800m by Gender and Grade", x="Difference in Times between Consecutive Grades on Log Scale", y=NULL) + 
  facet_grid(Grade~Gender, labeller=labeller(Grade = labels)) + theme(legend.position = 'none') +
  geom_text(x = 0.05, y = 12, aes(label = mean), data = gmeans, size=3)
...
gmeans <- slopes1600a %>% group_by(Gender, Grade) %>% summarise(mean = median(Slope,na.rm=T)*100)
gmeans$mean <- sprintf("Median Percent Change = %2.1f", gmeans$mean)
ggplot(data=slopes1600a,aes(x=Slope, fill=Gender)) + 
  geom_histogram(bins=50, fill="azure3")  + theme_bw() + geom_vline(xintercept = 0, linetype="dashed") +
  labs(title="Difference in Times for 1600m by Gender and Grade", x="Difference in Times between Consecutive Grades on Log Scale", y=NULL) + 
  facet_grid(Grade~Gender, labeller=labeller(Grade = labels)) + theme(legend.position = 'none') +
